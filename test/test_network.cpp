...
TEST_CASE("Message serialization and deserialization", "[message]")
{
    ...
    SECTION("Invalid message handling")
    {
        ...
        // Set remote UUID
        UUID remoteUuid = UUID();
        auto uuid = remoteUuid.getData();
        std::copy(uuid.begin(), uuid.end(), sizeMismatch.begin() + Network::MAGIC_NUMBER_SIZE);
        ...
        REQUIRE(deserialized.is<Network::Message::Broken>());
        REQUIRE(deserialized.getRemote() == remoteUuid); // Should have UUID from message
    }
}
